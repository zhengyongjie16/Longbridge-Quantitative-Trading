...
describe('getSellableOrders', () => {
  ...
  it('无订单时返回空结果', () => {
    const result = storage.getSellableOrders('TEST.HK', 'LONG', 1.0);
  ...
  it('includeAll=false 仅返回买入价 < 当前价的订单', () => {
    ...
    // 当前价 1.1，只有 O1(1.0) 和 O3(0.8) 低于当前价
  ...
  it('maxSellQuantity 限制时按低价优先整笔选单', () => {
    storage.setBuyOrdersListForLong('TEST.HK', [
      makeOrder('O1', 1.0, 100, 'TEST.HK', 1000),
    ...
    // 按低价优先: O2(0.8) → O3(0.9) → O1(1.0)，选前两个 = 200
    expect(result.totalQuantity).toBe(200);
    expect(result.orders).toHaveLength(2);
  ...
  it('防重过滤后再进行整笔截断', () => {
    storage.setBuyOrdersListForLong('TEST.HK', [
      ...
      makeOrder('O2', 0.9, 100, 'TEST.HK', 2000),
      makeOrder('O3', 1.0, 100, 'TEST.HK', 3000),
    ]);
...
// ========== getProfitableSellOrders 委托测试 ==========

describe('getProfitableSellOrders', () => {
  let storage: OrderStorage;
  ...
  it('默认行为与 getSellableOrders(includeAll=undefined) 一致', () => {
    storage.setBuyOrdersListForLong('TEST.HK', [
      makeOrder('O1', 1.0, 100),
      makeOrder('O2', 1.5, 100),
    ]);

    const r1 = storage.getProfitableSellOrders('TEST.HK', 'LONG', 1.2);
    const r2 = storage.getSellableOrders('TEST.HK', 'LONG', 1.2);

    expect(r1.totalQuantity).toBe(r2.totalQuantity);
    expect(r1.orders.length).toBe(r2.orders.length);
  });

  it('sellAll=true 等价于 getSellableOrders(includeAll=true)', () => {
    storage.setBuyOrdersListForLong('TEST.HK', [
      makeOrder('O1', 1.0, 100),
      makeOrder('O2', 1.5, 100),
    ]);

    const r1 = storage.getProfitableSellOrders('TEST.HK', 'LONG', 1.2, undefined, true);
    const r2 = storage.getSellableOrders('TEST.HK', 'LONG', 1.2, undefined, { includeAll: true });

    expect(r1.totalQuantity).toBe(r2.totalQuantity);
    expect(r1.orders.length).toBe(r2.orders.length);
  });

  it('sellAll=false 等价于 getSellableOrders(includeAll=false)', () => {
    storage.setBuyOrdersListForLong('TEST.HK', [
      makeOrder('O1', 1.0, 100),
      makeOrder('O2', 1.5, 100),
    ]);

    const r1 = storage.getProfitableSellOrders('TEST.HK', 'LONG', 1.2, undefined, false);
    const r2 = storage.getSellableOrders('TEST.HK', 'LONG', 1.2, undefined, { includeAll: false });
