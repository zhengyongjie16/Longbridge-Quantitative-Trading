 ...
 */
export function isInContinuousHKSession(date, isHalfDay = false) {
 ...
 */
export function isBeforeClose15Minutes(date, isHalfDay = false) {
  if (!date) return false;
  const hkTime = getHKTime(date);
  if (!hkTime) return false;
  const { hkHour, hkMinute } = hkTime;
 ...
 */
export function isBeforeClose5Minutes(date, isHalfDay = false) {
  if (!date) return false;
  const hkTime = getHKTime(date);
  if (!hkTime) return false;
  const { hkHour, hkMinute } = hkTime;
 ...
 */
export function hasChanged(current, last, threshold) {
  return (
