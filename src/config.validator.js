...
function validateTradingConfig() {
  ...
  // 验证监控标的
  if (!TRADING_CONFIG.monitorSymbol || (typeof TRADING_CONFIG.monitorSymbol === "string" && TRADING_CONFIG.monitorSymbol.trim() === "")) {
    errors.push("MONITOR_SYMBOL 未配置");
    missingFields.push("MONITOR_SYMBOL");
  }
  
  // 验证做多标的
  if (!TRADING_CONFIG.longSymbol || (typeof TRADING_CONFIG.longSymbol === "string" && TRADING_CONFIG.longSymbol.trim() === "")) {
    errors.push("LONG_SYMBOL 未配置");
    missingFields.push("LONG_SYMBOL");
  }
  
  // 验证做空标的
  if (!TRADING_CONFIG.shortSymbol || (typeof TRADING_CONFIG.shortSymbol === "string" && TRADING_CONFIG.shortSymbol.trim() === "")) {
    errors.push("SHORT_SYMBOL 未配置");
    missingFields.push("SHORT_SYMBOL");
  }
  
  // 验证目标买入金额
  if (!Number.isFinite(TRADING_CONFIG.targetNotional) || TRADING_CONFIG.targetNotional <= 0) {
    errors.push("TARGET_NOTIONAL 未配置或无效（必须为正数）");
    missingFields.push("TARGET_NOTIONAL");
  }
  
  // 验证最小买卖单位
  if (!Number.isFinite(TRADING_CONFIG.longLotSize) || TRADING_CONFIG.longLotSize <= 0) {
    errors.push("LONG_LOT_SIZE 未配置或无效（必须为正数）");
    missingFields.push("LONG_LOT_SIZE");
  }
  
  if (!Number.isFinite(TRADING_CONFIG.shortLotSize) || TRADING_CONFIG.shortLotSize <= 0) {
    errors.push("SHORT_LOT_SIZE 未配置或无效（必须为正数）");
    missingFields.push("SHORT_LOT_SIZE");
  }
  
  // 验证风险管理配置
  if (!Number.isFinite(TRADING_CONFIG.maxPositionNotional) || TRADING_CONFIG.maxPositionNotional <= 0) {
    errors.push("MAX_POSITION_NOTIONAL 未配置或无效（必须为正数）");
    missingFields.push("MAX_POSITION_NOTIONAL");
  }
  
  if (!Number.isFinite(TRADING_CONFIG.maxDailyLoss) || TRADING_CONFIG.maxDailyLoss < 0) {
...
export async function validateAllConfig() {
  ...
  logger.info(`  收盘前清仓: ${TRADING_CONFIG.clearPositionsBeforeClose ? "是" : "否"}`);
  logger.info("");
}
