...
- 开盘保护状态：早盘与午盘分别支持独立保护窗口；保护期内暂停信号生成，但不阻断自动寻标、自动换标和风险任务。
...
- `ADX`、`MACD`、`DIF`、`DEA`、`EMA:n` 不能写入任意 `SIGNAL_*`。
- `ADX` 的唯一合法入口是 `VERIFICATION_INDICATORS_BUY_*` 与 `VERIFICATION_INDICATORS_SELL_*`。
- `PSY:n` 可同时用于信号生成和延迟验证。
...
- 对于无方向性的趋势强度指标（例如 ADX），在 BUYCALL 与 SELLPUT 场景下，通过数值映射方式采用“整体低于初始值”的比较口径，用于确认从短暂脉冲回落到更平滑的趋势延续；BUYPUT 与 SELLCALL 场景按相反方向处理，但同样要求三个时间点在同一趋势方向上一致。
...
启动恢复：按时间顺序遍历当日保护性清仓记录，模拟触发-冷却周期，恢复当前周期触发计数、当前周期冷却起点以及最近一次已过期冷却边界，用于后续亏损分段协同。

午夜生命周期清理仅删除跨日模式冷却键并重置所有触发计数器；minutes 模式冷却不在此处清理，而是依赖时间自然过期。冷却自然到期时，由专门的生命周期协同模块统一消费冷却到期事件，并据此切换日内已实现亏损偏移的分段，保证新周期与旧周期的亏损口径边界清晰。
...
- 启动时会对空席位做一次与运行时一致的不阻塞主流程的寻标尝试；若仍处于早盘延迟窗口则直接跳过，不计入失败次数。寻标实际执行但未找到候选标的时，才累计当日失败次数并参与冻结判断。
