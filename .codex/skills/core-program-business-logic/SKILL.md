...
当自动寻标开启时，不再单独运行“静态标的距回收价清仓”定时任务，距回收价相关风险主要通过距离换标以及保护性清仓、牛熊证风险检查和末日保护等路径综合处理。
当自动寻标关闭时，保留一条按距回收价触发保护性清仓的监控链路：在满足清仓距离阈值时，为当前席位标的生成清仓信号并按清仓订单类型执行，执行成功后清空该标的订单记录并刷新浮亏及牛熊证相关缓存。
