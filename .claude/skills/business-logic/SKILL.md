...
买入信号（BUYCALL 和 BUYPUT）需要经过**延迟验证**，确认趋势后才执行买入。

### 3.1 验证配置

**延迟时间**：

- 默认 60 秒，可通过 `VERIFICATION_DELAY_SECONDS` 环境变量配置（范围 0-120 秒）
- 设置为 0 或未配置验证指标时，不进行延迟验证（立即执行）
...
- 例如：触发信号时间为 10:30:30，延迟 60 秒，需等待至 10:31:30
- 在等待期间，每秒记录一次当前监控标的的指标值到验证历史中（仅记录 triggerTime ± 5 秒窗口内的值）

**步骤 3：获取验证值**

- 到达延迟时间时，从验证历史中获取所有配置指标的第二个值（indicators2）
- **获取方式**：
...
```
监控标的当前价格 = 监控标的的最新价（从实时行情获取）
回收价 = callPrice（牛熊证的回收价）
距回收价百分比 = (监控标的当前价格 - 回收价) / 回收价 × 100%
...
- 如果监控标的价格异常小（< 1），可能是获取到了错误的价格（如牛熊证本身的价格），拒绝买入以确保安全
...
```
1. 交易时段检查（是否在交易时间）
2. 交易频率检查（同方向在配置的时间间隔内不能重复买入，通过 BUY_INTERVAL_SECONDS 配置，范围 10-600 秒，默认 60 秒，若不通过直接拒绝，不进行后续检查）
...
```
1. 检查交易日和交易时段
2. 获取实时行情数据（监控标的、做多标的、做空标的）
3. 计算技术指标（RSI6, MFI, KDJ, MACD）
4. 为待验证信号记录当前指标值（每秒记录一次，仅记录 triggerTime ± 5 秒窗口内的值）
5. 生成交易信号
   - 立即信号（卖出/平仓）
   - 延迟信号（买入/开仓）
6. 验证延迟信号（到达延迟时间后验证）
   - 从验证历史中获取所有配置指标的第二个值
   - 检查所有配置指标的第二个值是否满足验证条件
7. 处理立即信号（卖出）
   - 成本价判断
   - 智能平仓或全仓清仓
8. 处理验证通过的买入信号
...
9. 监控和修改未成交买入订单
10. 实时检查浮亏（仅在价格变化时检查）
    - 如果触发保护性清仓条件，立即执行市价单清仓
11. 显示变化的数据（监控标的指标、交易标的价格、账户持仓）
```
...
2. 等待订单成交
3. 订单成交后：
   - 本地更新订单记录（recordLocalSell，不再调用 API）
   - 刷新浮亏数据（refreshUnrealizedLossData）
   - 注意：保护性清仓后需要强制刷新订单记录（refreshOrders, forceRefresh=true）
4. 显示账户和持仓信息
```
...
| 参数                             | 说明                                         | 示例    |
| -------------------------------- | -------------------------------------------- | ------- |
| `MAX_POSITION_NOTIONAL`          | 单个标的最大持仓市值（HKD）                  | `50000` |
| `MAX_DAILY_LOSS`                 | 单个标的最大浮亏（HKD，正数）                | `3000`  |
| `MAX_UNREALIZED_LOSS_PER_SYMBOL` | 避难程序触发阈值（HKD，正数）                | `5000`  |
| `DOOMSDAY_PROTECTION`            | 末日保护程序（true/false）                   | `true`  |
| `BUY_INTERVAL_SECONDS`           | 同方向买入时间间隔（秒，范围 10-600，默认60）| `60`    |
...
| 参数              | 说明                              | 示例（默认值仅供参考）                   |
| ----------------- | --------------------------------- | ---------------------------------------- |
| `SIGNAL_BUYCALL`  | 买入做多信号配置（格式见第 2 节） | `(RSI6<20,MFI<15,D<20,J<-1)/3\|(J<-20)`  |
| `SIGNAL_SELLCALL` | 卖出做多信号配置（格式见第 2 节） | `(RSI6>80,MFI>85,D>79,J>100)/3\|(J>110)` |
| `SIGNAL_BUYPUT`   | 买入做空信号配置（格式见第 2 节） | `(RSI6>80,MFI>85,D>80,J>100)/3\|(J>120)` |
| `SIGNAL_SELLPUT`  | 卖出做空信号配置（格式见第 2 节） | `(RSI6<20,MFI<15,D<22,J<0)/3\|(J<-15)`   |
...
- 如果监控标的价格异常小（< 1），可能是获取到了错误的价格，拒绝买入以确保安全
...
- **v1.0.2** (2025-12-23): 新增 `BUY_INTERVAL_SECONDS` 配置项，同方向买入时间间隔可配置（范围 10-600 秒，默认 60 秒）
