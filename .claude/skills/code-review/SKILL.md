...
2. **候选生成**：可借助 `bun type-check`、`bun lint`；**必须**使用以下两个命令进行辅助检查：`bunx ts-prune`、`bunx knip --strict --use-tsconfig-files`。工具结果仅作候选，不做唯一依据。
...
- 每个小批次完成后立即执行 `bun type-check` 与 `bun lint`；若有测试，至少执行受影响模块测试（建议全量）。
