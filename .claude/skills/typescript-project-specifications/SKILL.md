...
31. **注释语言**：以中文为主，专有名词保留英文
32. **文件头注释**：新增 `.ts` 模块（除 `types.ts`、`utils.ts`、`types/`、`utils/` 下文件外）必须添加 `/** ... */` 文件头注释，描述"模块名 + 功能/职责/执行流程"
33. **类型注释**：`types.ts` / `types/` 下每个类型必须有独立块注释，说明用途、数据来源（如适用）和使用范围
34. **工具函数注释**：`utils.ts` / `utils/` 下每个函数必须有完整 JSDoc，包含功能说明、`@param`、`@returns`
35. **关键函数注释**：核心业务流程、状态机迁移、风控检查、生命周期处理、异步队列处理必须有函数注释，说明"做什么 + 为什么"，必要时补充副作用
36. **行内注释**：仅用于解释复杂业务判断或顺序约束，避免对显而易见代码重复描述
37. **注释与实现同步**：修改业务逻辑时必须同步更新注释，禁止注释与实现不一致
38. **测试注释**：保持轻量，重点说明场景意图、边界条件和业务期望

### 代码质量

39. **无兼容性代码**：不要编写兼容式、补丁式和临时性的代码，必须编写完整的系统性代码
40. **无临时或多余的注释**：不要编写临时性和多余的注释，例如此次代码更新的步骤或更新的内容（与代码无关）
41. **清除无用/临时代码**：不要保留无用/无效的代码或已弃用的代码，不要保留临时的测试文件
42. **命名语义一致**：函数/方法命名必须与实际行为一致；若行为扩展导致语义变化，新增语义正确的方法后必须全规模彻底替换旧方法并完全弃用，不得保留旧方法委托链，禁止仅靠注释解释语义偏差

### 流程要求

35. **完成检查**：编写完成后**必须**运行 `bun lint` 和 `bun type-check` 并修复所有问题
