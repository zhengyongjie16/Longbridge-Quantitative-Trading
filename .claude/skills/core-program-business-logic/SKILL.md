...
| 智能平仓 vs 全仓清仓     | 按成本均价决策卖出 vs 全部卖出                                                       |
| 正常卖出 vs 保护性清仓   | 前者按智能平仓/全仓规则处理，后者无条件清仓                                          |
| 立即信号 vs 延迟信号     | 无需验证直接执行 vs 需等待验证后执行                                                 |
| 距离换标 vs 周期换标     | 前者由距回收价越界触发，可包含移仓卖出/回补；后者按席位就绪时长触发，仅在空仓时换标  |
...
- **启动时**：拉取全量订单（历史与当日、按订单编号去重）后，按**当前席位标的**逐标的筛选该标的的买卖订单并应用过滤算法；静态模式席位为配置标的，自动寻标模式席位仅在有持仓时恢复对应标的，无持仓方向保持 EMPTY 并由后续寻标补齐
- **交易后**：本地更新，不调 API
...
2. **判断整体盈亏**：
   - 整体盈利（当前价 > 成本均价）→ 卖出全部可卖订单（仍受待成交占用与可用持仓约束）
...
买入信号先经**风险检查冷却**（同标的买入通道 10 秒内仅允许一信号进入下述检查，冷却期内跳过；BUYCALL 和 BUYPUT 共享同一冷却键），再依次通过以下 6 项检查（任一失败则拒绝）：
...
自动寻标用于在**不依赖静态交易标的**的前提下，为每个监控标的的做多/做空方向动态分配"当前可交易标的（牛/熊证）"；自动换标支持两类触发：**距回收价越界触发（距离换标）**与**席位就绪时长到期触发（周期换标）**。
...
- **允许时机**：仅在允许运行的交易时段内执行；自动寻标时机不受全局开盘保护开关限制，但受早盘开盘延迟约束。
- **开盘延迟**：仅早盘生效；若该方向已有持仓则可跳过延迟。
- **触发冷却**：同一方向 10 分钟内最多触发一次寻标（席位为空时的重试也受此冷却约束）。
- **失败计数与冻结机制**：
  - 每次寻标失败（数据源失败/无候选/流程异常）将失败计数加 1
...
- **结果处理**：寻标失败 → 更新失败计数，达到上限则冻结席位；成功则占位并在后续主循环中纳入行情订阅集合，失败计数清零。
- **启动时推断**：启动时优先使用已有持仓的标的，避免自动寻标覆盖现有仓位；若无持仓则保持 EMPTY，等待后续自动寻标。
- **行情未就绪**：席位已占用但尚未收到首个行情时，买入信号会以"行情未就绪"被丢弃（不按席位空处理）；卖出可尝试执行，失败按失败处理。

**自动换标（Auto Switch）**

- **触发入口**：
  - 距离换标：仅当监控标的价格变化达到阈值时检查，触发来源为"距回收价百分比越界"
  - 周期换标：每秒按席位最近进入就绪状态的时间与当前交易分钟差检查；当周期换标间隔分钟数小于等于 0 时禁用
- **流程推进**：一旦进入换标流程，后续会持续推进直至完成或失败；存在进行中的换标时，即使价格未变化也会继续调度推进。
- **越界判定（距离换标）**：牛/熊方向分别配置一个触发区间（区间下限与上限）；距回收价百分比小于等于区间下限或大于等于区间上限时触发换标（即落在区间外或恰在边界上即触发）。
- **预寻标 + 日内抑制（同标的循环保护）**
  - 越界后先预寻标候选：若候选与旧标的一致 → 记录"当日抑制"并停止换标（不清席位、不递增版本）。
  - 当日抑制按"方向 + 旧标的 + 港股日期"记录；跨日后清理，次日可再次触发。
- **距离换标状态机（撤单 → 持仓处理 → 占位 → 可选回补）**
...
- **周期换标规则**
  - 仅在自动寻标开启且周期换标间隔分钟数大于 0 时生效；受"可交易时段"与"开盘保护"门禁共同约束
  - 计时口径使用"交易分钟差"（席位就绪分钟到当前分钟），午休不计入有效分钟
  - 到期后若仍有买入订单记录，则进入等待空仓状态；检测到空仓后才真正启动换标
  - 周期换标仅做标的替换，不执行移仓卖出与回补买入；其状态流转为 CANCEL_PENDING → BIND_NEW → COMPLETE
  - 周期模式撤单同样只处理旧标的未完成买入挂单；若已有距离换标在跑，距离换标优先
...
5. 初始化"席位体系"：结合持仓与订单归属恢复席位（自动寻标模式仅恢复有持仓方向），随后对空席位执行一次非阻塞寻标
6. 回放交易日志，恢复清仓冷却状态
...
1. **交易日生命周期管理**：检测跨日（交易日标识变化）触发午夜清理，检测开盘触发开盘重建（见 7.4 节）

2. **交易日检查**：首次或跨天时调用 API，之后使用缓存

3. **交易时段检查**：判断是否在连续交易时段，不在则跳过（退出连续交易时段时清理所有待验证信号）

4. **开盘保护检查**：早盘或午盘开盘保护窗口内跳过信号生成（用于降低开盘波动带来的假信号风险，午盘保护在半日市不生效；自动寻标与风控任务仍可按各自规则运行）
...
1. 从行情数据中提取当前标的的行情
2. 席位同步与自动寻标/换标调度：每秒调度双方向自动寻标与周期换标任务；价格变化或存在进行中换标时调度距离换标检查任务并推进距离换标状态机
3. 监控价格变化
4. 浮亏检查（仅在价格变化时）
5. 获取 K 线数据
6. 计算技术指标
...
2. **行情域**：重置所有运行时行情订阅与内部缓存
3. **席位域**：重置所有监控标的的自动换标状态，清空轮证列表缓存，清空所有席位绑定（保留切换/寻标时间戳，重置席位就绪时间戳、失败计数和冻结标记），递增席位版本号
4. **订单域**：重置交易执行器的运行时状态（订单追踪、持有标的集合等）
5. **风控域**：重置风险检查冷却，重置日内亏损追踪器，清除跨日模式的清仓冷却键（保留分钟模式），清空浮亏数据和牛熊证风险信息缓存
...
2. **风控域**：无独立重建动作（由统一重建流水线按当日数据重建）
3. **订单域**：无独立重建动作（由统一重建流水线从全量订单 API 重建）
4. **席位域**：无独立重建动作（由统一重建流水线按启动席位逻辑恢复）
5. **行情域**：无独立重建动作（由统一重建流水线重建行情订阅）
6. **信号运行时域**：重启所有异步处理器，刷新门禁版本标记为最新
...
- 智能平仓开关：决定卖出信号按"成本均价决策卖出（整体盈利可全卖，未盈利仅卖盈利订单）"还是"直接全仓"
...
- 周期换标间隔分钟数：大于 0 启用周期换标，0 关闭（上限 120 分钟）
...
| 智能平仓          | 基于成本均价判断，整体盈利可全卖，未盈利仅卖盈利部分                                                      |
| 保护性清仓        | 浮亏超限紧急清仓                                                                                          |
| 末日保护          | 收盘前自动保护机制                                                                                        |
| ELO               | 增强限价单                                                                                                |
| MO                | 市价单                                                                                                    |
...
| 自动换标          | 包含距离换标与周期换标：前者按距回收价越界触发（可含移仓/回补），后者按席位就绪时长到期触发（空仓后换标） |
| 周期换标等待态    | 周期换标到期但仍有买入订单记录时的等待状态，检测到空仓后继续换标流程                                      |
