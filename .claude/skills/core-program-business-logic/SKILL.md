...
- 有盈利订单 → 可卖出数量为盈利订单中未被待成交卖出占用的部分，卖出数量取该可卖数量与可用持仓量的较小值；超出时按买入价从低到高整笔选单（不拆分订单，实际卖出数量可能小于限制数量）；提交卖出时登记关联买入订单用于防重（见 6.1）
...
使用 WebSocket 订阅订单状态变化。**提交卖出订单时**登记待成交卖出及关联买入订单标识（存储在信号对象的关联买入订单标识列表中），用于智能平仓防重；部分成交时更新已成交数量；完全成交或撤单时移除该登记（撤单时返回的记录状态标记为已取消）。订单完全成交时：
1. 使用实际成交价更新本地订单记录
