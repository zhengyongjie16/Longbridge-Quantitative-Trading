...
| 信号类型 | 条件 | 处理方式 |
|---------|------|---------|
| 立即信号 | 延迟时间=0 或 验证指标为空 | 直接推入买入/卖出任务队列 |
| 延迟信号 | 延迟时间>0 且 验证指标非空 | 推入延迟验证器，验证通过后再推入对应队列 |
...
**示例**：假设当前价为 1.20 港币，可用持仓 300 股。存在两笔盈利订单：100 股买入价 1.00、250 股买入价 1.10，盈利订单总量为 350 股。最终卖出数量取盈利订单总量与可用持仓的较小值，即 300 股。

**常见误区**：
- 认为智能平仓会因成本价盈利而全仓清空（当前逻辑不会）
- 忽略订单记录不可用或无盈利订单会导致“不执行卖出”
...
| 开仓成本 R1 | 未平仓买入订单的（成交价 × 成交数量）之和 | 浮亏监控 |
...
**风险检查冷却**：同标的同方向 10 秒内只允许一个信号进入风险检查，冷却期内跳过。
...
**计算口径**：浮亏 = R2 - R1（使用“开仓成本 R1”口径进行风控）
- **基础 R1**：未平仓买入订单市值总和（每笔 = 成交价 × 成交数量）
- **R2**：当前价 × 持仓量(N1)
- **N1**：未平仓买入订单的成交数量总和
- **当日已实现亏损偏移**：系统会将当日已实现亏损作为偏移量对 R1 做调整（R1 取更保守口径），从而在风险恶化时更早触发保护性清仓
...
**清仓冷却追踪**：保护性清仓发生后会记录清仓事件并进入冷却期；冷却期内买入信号会在“清仓冷却”检查中被拒绝（用于避免刚清仓后立刻再次开仓）
...
- **筛选原则（文字口径）**：基于牛/熊证候选列表，计算“分均成交额（成交额按已开盘分钟数折算）”，并按以下规则筛选与选优：
  ...
  - 选优：|距回收价百分比|更小优先；相同则分均成交额更高优先；仅取最优 1 个
...
- **越界判定**：牛/熊方向分别配置一个触发区间（区间下限与上限），包含等于边界（落在区间外即触发）。
- **预寻标 + 日内抑制（同标的循环保护）**
...
3. 为每个监控标的创建独立上下文：策略、订单记录器、延迟信号验证器、风险检查器、浮亏监控器、自动寻标管理器（如启用）
4. 初始化“席位体系”：为每个监控标的的做多/做空方向建立席位状态与席位版本号（作为动态交易标的的唯一来源）
5. 获取账户和持仓信息，并初始化持仓快速查询缓存
6. 初始化订单记录：启动时合并历史订单与当日订单并解析归属；静态标的模式按配置过滤，自动寻标模式按“该方向最新成交标的 + 持仓验证”初始化席位（无历史且无持仓则席位为空，等待后续寻标）
7. 初始化浮亏监控数据（基于订单记录/持仓）
8. 初始化牛熊证信息：对当前席位标的刷新回收价缓存（换标后也会按同一入口刷新）
9. 为每个延迟验证器注册回调（验证通过后推入对应队列，并校验席位版本号）
10. 启动买入/卖出处理器、监控任务处理器与主循环
...
- 距回收价换标区间（牛/熊分别配置）：用“区间下限与上限”描述触发范围，且包含边界；越界时进入换标流程
...
| 信号触发时间 | 立即信号=生成时间，延迟信号=T0，末日保护=生成时间 |
...
### 信号对象生命周期（易错点）
- **立即信号**：入队后由买入/卖出处理器消费并统一释放；若处于非连续交易时段则直接丢弃并释放
