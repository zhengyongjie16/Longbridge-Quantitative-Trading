...
1. **排序卖出订单**：将所有卖出订单按成交时间从旧到新排序（D1→DN）

2. **识别M0集合**：获取成交时间 > 最新卖出订单时间的买入订单（这些订单未经历任何卖出）
...
3. **从旧到新处理每个卖出订单**（D1→DN，顺序不可颠倒）：
   - 对于卖出订单Di：
     ...
     - 计算这些买入订单的总数量
     - **判断卖出类型**：
       - 若卖出数量 ≥ 买入总量：全部卖出，移除这些买入订单
       - 若卖出数量 < 买入总量：部分卖出，保留成交价 ≥ 卖出价的买入订单（盈利部分）
     - 保留成交时间在Di和Di+1之间的买入订单（未被Di影响）

4. **合并结果**：最终记录 = M0 + 经过所有卖出订单过滤后的买入订单

**关键约束**：
- 必须按时间顺序从旧到新处理卖出订单，不可倒序或乱序
- 每个卖出订单的过滤结果会影响下一个卖出订单的处理
- 这是累积过滤过程，后面的卖出订单基于前面过滤后的结果继续过滤

**示例**：
```
买入订单：B1(10元,100股,T1) B2(11元,100股,T2) B3(12元,100股,T3)
卖出订单：S1(10.5元,150股,T4) S2(11.5元,50股,T5)

处理S1：卖出150股，买入总量300股 < 卖出量，保留价格≥10.5的订单 → 保留B2,B3
处理S2：基于B2,B3继续过滤，卖出50股，买入总量200股 < 卖出量，保留价格≥11.5的订单 → 保留B3
最终记录：B3(12元,100股)
```
...
- 每个卖出订单的过滤结果影响下一个卖出订单
- 准确识别未平仓高价买入订单
- 算法实现在独立的`orderFilteringEngine.ts`模块中
