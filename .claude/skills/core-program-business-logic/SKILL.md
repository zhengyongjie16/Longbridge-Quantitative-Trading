...
| 概念对 | 区别 |
|--------|------|
| 平摊成本价 vs 开仓成本 | 前者判断盈利（智能平仓），后者计算浮亏（R1） |
...
|------|------|
| 当前价 > 成本价 | 全仓清仓 |
| 当前价 ≤ 成本价 | 仅卖出买入价低于当前价的历史订单 |
| 无符合条件订单 | 设为 HOLD，跳过此信号 |

### 4.3 成本价类型

| 类型 | 计算方式 | 用途 |
|------|---------|------|
| 平摊成本价 | (买入额 - 卖出额) / 持仓量 | 判断是否盈利 |
...
通过 `MONITOR_COUNT` 指定监控标的数量，每个监控标的使用后缀 `_N`（N 从 1 开始）区分。
...
### 信号触发时间（triggerTime）
- **统一字段**：所有信号类型使用 `triggerTime` 记录触发时间
- **立即信号**：`triggerTime = 信号生成时间`
- **延迟信号**：`triggerTime = 延迟验证基准时间（T0）`
- **末日保护信号**：`triggerTime = 信号生成时间`
- **用途**：交易日志追溯、延迟验证计算
- **废弃字段**：`signalTriggerTime` 已废弃，仅保留兼容性
