...
**执行时机**：信号加入验证器后，定时器在 T2（即 T0 + 10 秒）时执行验证（确保 T2 数据已记录）。
...
**验证结果**：
- 所有验证指标在所有 3 个时间点都满足条件 → 验证通过，推入对应队列执行（并校验席位版本号）
...
- **清理时机**：退出连续交易时段时会清理所有待验证信号（不仅限于收盘）；标的切换/方向取消时也会清理对应待验证信号，避免过期验证；信号验证通过或失败后由验证器释放对象
...
买入信号先经**风险检查冷却**（同标的同动作类型 10 秒内仅允许一信号进入下述检查，冷却期内跳过；买入做多和买入做空共享同一冷却键，卖出做多和卖出做空共享同一冷却键），再依次通过以下 6 项检查（任一失败则拒绝）：
...
- **R2**：当前价乘以持仓量 N1
- **N1**：未平仓买入订单的成交数量总和
- **当日已实现盈亏偏移**：系统根据当日已实现盈亏对开仓成本 R1 做调整。计算公式：当日已实现盈亏 = 总卖出额 - (总买入额 - 未平仓买入成本)。当日已实现亏损时（负值），调整后 R1 = max(0, 基础 R1 - 偏移值)，使 R1 增大，浮亏口径相对宽松，避免已亏损后因短期波动过早触发清仓
...
使用 WebSocket 订阅订单状态变化。**提交卖出订单时**登记待成交卖出及关联买入订单标识（存储在信号对象的关联买入订单标识列表中），用于智能平仓防重；部分成交时更新已成交数量；完全成交或撤单时移除该登记。订单完全成交时：
1. 使用实际成交价更新本地订单记录
...
**处理流程**：
1. 从队列获取任务
2. 校验席位版本号：若信号携带的席位版本号与当前席位版本号不匹配，或信号标的与当前席位标的不一致，则丢弃信号并记录原因
3. 获取监控上下文（行情、持仓数据从缓存获取；卖出处理器使用 positionCache，买入处理器在风险检查时实时获取账户和持仓）
4. 卖出处理器：直接计算卖出数量（不经过风险检查）
5. 买入处理器：执行风险检查（需实时调用 API 获取账户和持仓）
6. 提交订单执行
7. 释放信号对象（回对象池）
...
**生命周期状态机**：
- 活跃状态 → 午夜清理中 → 午夜清理完成 → 开盘重建中 → 活跃状态
- 失败时自动重试（指数退避策略），状态回到午夜清理中或开盘重建失败
...
清理完成后：设置待开盘重建标记为真，记录目标交易日标识，状态转为午夜清理完成。
...
重建完成后：清除待开盘重建标记，状态转为活跃状态，恢复交易。

**失败重试机制**：
- 午夜清理失败：指数退避重试（首次 30 秒，最大倍数 16），状态保持午夜清理中
- 开盘重建失败：指数退避重试（首次 30 秒，最大倍数 16），状态转为开盘重建失败，交易保持禁止
...
### 信号对象生命周期（易错点）
- **立即信号**：仅在连续交易时段内由主循环分流入队；入队后由买入/卖出处理器消费并统一释放；若被风险检查冷却跳过，在主循环中被识别并释放
- **延迟信号**：验证失败/超时/缺失数据/退出连续交易时段/席位版本不匹配等情形由验证器丢弃并释放；验证通过后进入队列，由处理器统一释放；验证通过时需校验席位版本号和标的匹配性
