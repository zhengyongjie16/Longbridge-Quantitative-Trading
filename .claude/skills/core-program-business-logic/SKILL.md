...
- `/N`：需满足N项，不设则全部满足
- `|`：满足任一组即可
- **支持指标**：RSI:n、PSY:n、MFI、K/D/J（MACD/DIF/DEA/EMA:n 仅用于延迟验证）
- **运算符**：`<`、`>`，支持负数如`J<-20`
...
- **验证指标**：可配置 K/D/J/MACD/DIF/DEA/EMA:n/PSY:n
...
- **买入超时**：超过配置时间（默认 3 分钟）未成交，自动撤销（避免追高）
- **卖出超时**：超过配置时间（默认 3 分钟）未成交，撤销后转市价单重新委托
