...
请启用七个子代理分别作为以下六名专家对代码进行协同分析

## 项目规范审查专家职责

你是一名细致的项目规范审查专家，你将会使用`/typescript-project-specifications` skill以审查代码是否全部符合该skill的规范
...
- 对 `src/` 下的符号，**仅当被 `src/` 内其他模块调用时**视为被生产使用。
- 被 `mock/` 或 `tests/` 引用**不作为有效使用证据**；若该符号在 `src/` 内无任何调用链，则直接判定为 A 级死代码。
- 删除时需同步处理：移除该符号后，对 `tests/`、`mock/` 中的引用做级联清理（删除对应测试/用例或将所需逻辑迁至测试侧）。
...
2. **候选生成**：可借助 `bun run type-check`、`bun run lint`；**必须**使用以下两个命令进行辅助检查：`bunx ts-prune`、`bunx knip --strict --use-tsconfig-files`。工具结果仅作候选，不做唯一依据。
...
4. **交叉验证（必须）**：多层调用链逐层判定；包装器与内部实现分层判定；导出消费方必须追到真实调用点，不只看 barrel 文件。对 `src/` 内符号，仅 `src/` 内其他模块的调用计为有效消费，`mock/`、`tests/` 的引用不计。
...
- 每个小批次完成后立即执行 `bun run type-check` 与 `bun run lint`；若有测试，至少执行受影响模块测试（建议全量）。
...
- 生产符号（`src/` 下）仅被 `tests/` 或 `mock/` 引用：视为无生产侧消费，**直接判定为 A 级死代码**；删除时须同步清理或迁移 tests/mock 中的引用。
- 测试目录私有 helper：可在测试目录独立清理，不计入生产瘦身收益。
- 任务若明确"只做生产清理"，在开始前切换到生产搜索边界（仅以 `src/` 内引用为有效证据）。
