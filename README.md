...
| 交易日生命周期管理 | 自动检测跨日并执行午夜清理（清空运行时状态），交易日开盘时自动重建（恢复账户/持仓/订单/席位，并从成交日志恢复清仓冷却与亏损分段），支持失败重试 |
...
> **注意**：环境变量中的 `N` 表示监控标的索引（如 `_1`、`_2`）。买入和卖出的延迟验证时间默认 `60` 秒；验证指标未配置时，对应方向延迟验证关闭。`ADX` 仅支持出现在 `VERIFICATION_INDICATORS_*` 中，不支持出现在 `SIGNAL_*` 信号条件里。若验证指标包含 `ADX` 这类无方向性趋势强度指标，系统会先做数值映射，再并入对应的上涨/下跌趋势判断；其实际比较目标统一为 `ADX_t < ADX_0`。
...
4. **风险检查**：频率限制、清仓冷却（频率通过后会立即记录买入尝试）、价格限制、末日保护、牛熊证风险、持仓/现金限制
