...
- **买入操作**：
  - 同方向标的：在配置的时间间隔内只能买入一次（通过 `BUY_INTERVAL_SECONDS` 配置，范围 10-600 秒，默认 60 秒）
  - 例如：配置为 60 秒时，买入做多标的后，在 60 秒内再次买入做多标的会被拒绝
  - 不同方向标的：不互相限制（例如买入做多标的后，可以立即买入做空标的）
- **卖出操作**：不受频率限制
...
| `BUY_INTERVAL_SECONDS`           | 同方向买入时间间隔（秒，范围 10-600）                       | `60`        |
