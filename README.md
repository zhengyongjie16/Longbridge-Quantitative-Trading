...
- ✅ **末日保护程序**：可配置在收盘前 30 分钟拒绝买入，收盘前 5 分钟自动清空所有持仓
...
1. **立即记录**：记录当前的 KDJ.J 值（记为 J1）和 MACD 值（记为 MACD1）
2. **等待 60 秒**：例如触发信号的时间为 10:30:30，需等待至 10:31:30
3. **获取验证值**：到达 60 秒时获取当前 KDJ.J 值（记为 J2）和 MACD 值（记为 MACD2）
   - 优先从实时监控标的的值获取（精确匹配目标时间）
   - 若获取失败（时间戳不匹配），则获取距离当前时间最近的值（误差在 5 秒内）
4. **验证条件**：若 J2 > J1 且 MACD2 > MACD1，则验证通过，执行买入
...
- **收盘前 30 分钟拒绝买入**：当末日保护程序启用时，收盘前 30 分钟内禁止所有买入操作（卖出操作不受影响）
  - 正常交易日：15:30-16:00 拒绝买入
  - 半日交易日：11:30-12:00 拒绝买入
...
CLEAR_POSITIONS_BEFORE_CLOSE=true  # 末日保护程序（收盘前30分钟拒绝买入，收盘前5分钟清仓）
...
4. **末日保护程序**（可选）
   - 收盘前 30 分钟拒绝买入：正常交易日 15:30-16:00，半日交易日 11:30-12:00
...
| `CLEAR_POSITIONS_BEFORE_CLOSE` | 末日保护程序（收盘前 30 分钟拒绝买入，收盘前 5 分钟清仓） | `true` 或 `false` |
...
- ✅ **完善验证阶段策略**：
  - 详细说明 60 秒延迟验证的具体流程
  - 明确 J1、MACD1 的记录时机和 J2、MACD2 的获取方式（优先精确匹配，失败则获取 5 秒内最近值）
...
- ✅ 末日保护程序（收盘前 30 分钟拒绝买入，收盘前 5 分钟清仓）
- ✅ 完整的配置验证和错误处理
- ✅ 详细的日志和交易记录
