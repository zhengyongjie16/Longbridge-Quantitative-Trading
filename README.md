...
### 作者说明（必读）
1. 该程序一般不会交易正股（正股仅用作实时分析，即配置中的监控标的），而是在轮证上进行多空交易，主要为交易做多或做空方向的权证或牛熊证。
2. 目前的交易策略仅针对日内交易，当前也仅支持日内交易。
3. 请务必掌握港股、窝轮以及技术指标的相关知识，窝轮自带杠杆属性且还存在到期时间和回收等机制，这些因素都存在较大风险。
4. 请务必掌握代码知识（主要为typescript），该程序不建议非开发者使用。
5. 该程序代码主要由claude编写，也推荐使用claude进行优化和再开发，这是当前最强大的模型。
4. 请务必先使用模拟账户进行调试.
...
| 参数                                  | 默认值 | 说明                     |
| ------------------------------------- | ------ | ------------------------ |
| `MAX_UNREALIZED_LOSS_PER_SYMBOL_N`    | `0`    | 单标浮亏保护阈值         |
| `VERIFICATION_DELAY_SECONDS_BUY_N`    | `60`   | 买入延迟验证时间（秒）   |
| `VERIFICATION_INDICATORS_BUY_N`       | `D,DIF` | 买入验证指标（逗号分隔） |
| `VERIFICATION_DELAY_SECONDS_SELL_N`   | `60`   | 卖出延迟验证时间（秒）   |
| `VERIFICATION_INDICATORS_SELL_N`      | `D,DIF` | 卖出验证指标（逗号分隔） |
| `BUY_INTERVAL_SECONDS_N`              | `60`   | 同向买入间隔（秒）       |
