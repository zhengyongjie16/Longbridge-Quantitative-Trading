...
系统当前采用“事件驱动 + 时间驱动”协同架构：分钟级 K 线更新负责推进指标与普通信号生成，标准化 quote 事件负责浮亏风控、距离换标与静态清仓唤醒，主循环则专注交易时段门禁、生命周期、末日保护与监控任务调度。监控标的通常用于分析，不一定直接作为实际成交标的。
...
同时监控多个标的，每个标的都拥有独立配置。系统会在监控标的的分钟级 K 线更新事件到达时推进指标管线，并按配置生成四类普通信号：`BUYCALL`、`SELLCALL`、`BUYPUT`、`SELLPUT`。

当启用延迟验证时，普通信号不会直接进入执行队列，而是先进入 `DelayedSignalVerifier`；验证通过后，再按买卖方向分流到 `BuyTaskQueue` 或 `SellTaskQueue`。因此，普通信号生成与时间主循环已解耦，主循环不再负责常规指标推进。开盘保护只短路普通信号生成与延迟验证登记，不作为启动、订阅、自动寻标、换标或风险运行时的整体阻断。
...
除普通信号外，系统还维护多个事件运行时：稳态 quote 订阅由 `QuoteSubscriptionRuntime` 统一拥有，并按监控标的、席位绑定、持仓、在途订单和临时等待 retain 汇总成订阅集合；quote 事件会分别进入浮亏风控链路与 monitor quote 链路，前者在生命周期门禁、freshness baseline 与席位路由校验后触发保护性风控，后者在自动寻标开启时驱动距离换标，在自动寻标关闭时接管静态清仓等待唤醒；成交后一致性运行时会在账户、持仓、保护性清仓状态和浮亏缓存刷新完成后，再放行后续执行链路。

### 3. 席位、自动寻标与交易日生命周期

启用自动寻标后，系统会为每个监控标的维护 LONG / SHORT 两个方向的席位，交易标的不再写死，而是由运行时根据候选筛选、成交额、距回收价、到期月等约束动态决定。空席位由席位变化、交易门禁变化和一次性定时器唤醒自动寻标；周期换标由主循环调度 `AUTO_SYMBOL_TICK` 任务；距离换标由 monitor quote 事件触发，并与周期换标共用撤单、卖出、绑定新标、等待行情、回补和完成的换标状态机。

系统还内置交易日生命周期管理：启动阶段先完成配置、认证、行情客户端与 post-gate 运行时装配，再尝试加载 runtime snapshot。若启动快照或初次重建失败，程序不会退出，而是保持主循环和生命周期管理器运行，暂不启动交易相关事件运行时，并在连续交易时段通过开盘重建重试恢复。正常运行中会在跨日时执行午夜清理，在开盘后执行重建，恢复账户、持仓、订单记录、风险缓存、行情订阅与席位状态，并在重建完成前关闭交易门禁，避免脏状态延续到下一交易日。
...
```mermaid
flowchart TD
    A[启动程序] --> B[加载配置 认证 创建 pre-gate runtime]
    B --> C[创建 post-gate runtime]
    C --> D{启动快照和初次重建是否成功}
    D -- 是 --> E[启动订阅 事件 runtime 与异步处理器]
    D -- 否 --> F[保持主循环运行并等待开盘重建]
    F --> G[生命周期管理器触发开盘重建]
    G --> E

    E --> H[主循环 tick]
    H --> I[处理交易时段门禁 生命周期 末日保护]
    H --> J[处理监控任务与周期换标]
    ...
    M --> N{是否启用延迟验证}
    N -- 否 --> O[进入买卖任务队列]
    N -- 是 --> P[DelayedSignalVerifier]
    P --> O

    Q[Quote 更新事件] --> R[浮亏风控 / 距离换标 / 静态清仓唤醒]
    O --> S[买卖处理器执行订单]
    S --> T[订单监控与成交后一致性刷新]
    T --> H
```
...
- 普通信号由分钟 K 线事件驱动，主循环不直接负责常规指标计算与信号生成，开盘保护只影响这条普通信号链路
- Quote 订阅由 retain 投影统一管理；Quote 事件负责浮亏风控、距离换标和静态清仓等待唤醒等实时链路，并在执行前复用生命周期与成交后一致性门禁
- 主循环主要承担时间语义：交易时段门禁、生命周期、末日保护与周期换标任务调度
- 启动快照或初次重建失败不会直接退出，而是保持主循环运行，并切换到等待开盘重建的恢复分支
