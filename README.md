...
当启用延迟验证时，普通信号不会直接进入执行队列，而是先进入 `DelayedSignalVerifier`；验证通过后，再按买卖方向分流到 `BuyTaskQueue` 或 `SellTaskQueue`。因此，普通信号生成与时间主循环已解耦，主循环不再负责常规指标推进。
...
除普通信号外，系统还维护多个事件运行时：quote 事件会分别进入浮亏风控链路与 monitor quote 链路，前者负责保护性风控，后者负责距离换标与静态清仓等待唤醒；成交后一致性运行时会在账户/持仓/浮亏刷新完成后，再放行后续执行链路。

### 3. 席位、自动寻标与交易日生命周期

启用自动寻标后，系统会为每个监控标的维护 LONG / SHORT 两个方向的席位，交易标的不再写死，而是由运行时根据候选筛选、成交额、距回收价、到期月等约束动态决定。自动寻标、周期换标与距离换标共用统一的席位状态机推进。

系统还内置交易日生命周期管理：启动时先尝试加载 runtime snapshot；若启动快照失败，则进入“阻断交易并等待开盘重建”的恢复分支。正常运行中会在跨日时执行午夜清理，在开盘后执行重建，恢复账户、持仓、订单记录、风险缓存与席位状态，并在重建完成前统一关闭交易门禁，避免脏状态延续到下一交易日。
...
系统将启动阶段与运行阶段的门禁拆成两组独立配置：

- `STARTUP_GATE_MODE`
- `RUNTIME_GATE_MODE`

仅当显式设置为 `skip` 时才会跳过对应门禁；未设置或填其他值时都会按 `strict` 处理。`RUN_MODE=dev` 只表达开发模式语义，不再隐式跳过门禁。
...
```mermaid
flowchart TD
    A[启动程序] --> B[加载配置 认证与 startup gate]
    B --> C[创建 pre/post-gate runtime]
    C --> D{启动快照是否成功}
    D -- 是 --> E[执行初次重建并启动各事件 runtime]
    D -- 否 --> F[阻断交易并等待开盘重建]
    F --> G[生命周期管理器触发开盘重建]
    G --> E
...
- 普通信号由分钟 K 线事件驱动，主循环不直接负责常规指标计算与信号生成
- Quote 事件负责风险、距离换标和静态清仓等待唤醒等实时链路
- 主循环主要承担时间语义：交易时段门禁、生命周期、末日保护与监控任务调度
- 启动快照失败不会直接退出，而是切换到等待开盘重建的恢复分支
