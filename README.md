...
基于 LongPort OpenAPI / Node.js / TypeScript 的港股自动化量化交易系统，通过监控目标资产（如恒生指数）的技术指标，在牛熊证上自动执行做多/做空交易。支持多指标组合策略、延迟验证、风险控制和订单管理。
...
| SELLCALL | 卖出做多 | `SIGNAL_SELLCALL` | 延迟 60 秒验证趋势：T0、T0+5s、T0+10s 指标值均需小于初值 |
| BUYPUT   | 买入做空 | `SIGNAL_BUYPUT`   | 延迟 60 秒验证趋势：T0、T0+5s、T0+10s 指标值均需小于初值 |
| SELLPUT  | 卖出做空 | `SIGNAL_SELLPUT`  | 延迟 60 秒验证趋势：T0、T0+5s、T0+10s 指标值均需大于初值 |
...
4. 生成延迟验证信号（买入和卖出）
5. 记录待验证信号的指标历史值
6. 验证到期信号（T0+15秒后验证，检查T0/T0+5s/T0+10s趋势）
7. 风险检查（买入：频率/价格/末日/牛熊证/浮亏/持仓）
8. 处理卖出信号（成本价判断和数量计算）
9. 执行订单（ELO限价单/MO市价单）
