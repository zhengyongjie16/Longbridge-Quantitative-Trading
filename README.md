...
- **条件 1**：四个指标中至少 3 个满足 且 监控标的价格 < VWAP
  - RSI6 < 20
  - RSI12 < 20
  - KDJ.D < 20
  - KDJ.J < -1
- **条件 2**：KDJ.J < -20（无需检查均价）
...
- **条件 1**：四个指标中至少 3 个满足 且 做多标的价格 > 持仓成本价
  - RSI6 > 80
  - RSI12 > 80
  - KDJ.D > 79（注意：不是 80）
  - KDJ.J > 100
...
- **条件 1**：四个指标中至少 3 个满足 且 做空标的价格 > 持仓成本价
  - RSI6 < 20
  - RSI12 < 20
  - KDJ.D < 22（注意：不是 20）
  - KDJ.J < 0
- **条件 2**：KDJ.J < -15（无需检查均价）
...
3. 如果有卖出订单：
   ...
   - **MN**：从最旧的卖出订单开始，依次判断和过滤：
     - 对于每个卖出订单（按成交时间从旧到新排序）：
       - 获取成交时间 < 该卖出订单成交时间的买入订单
       - 如果卖出订单的成交数量 ≥ 这些买入订单的总数量，则这些买入订单全部被卖出
       - 否则，从这些买入订单中过滤出成交价 ≥ 卖出价的买入订单
       - 将这些过滤出的买入订单 + 成交时间 > 当前卖出订单时间 且 < 下一个卖出订单时间的买入订单 = 新的候选列表
     - 最终订单列表 = M0 + MN
