...
async function testCachedCandidateAfterSell() {
  ...
  await autoSymbolManager.maybeSwitchOnDistance({
    direction: 'LONG',
    monitorPrice: 20000,
    quotesMap: new Map([['OLD', { price: 0.1, lotSize: 100, name: 'OLD', symbol: 'OLD' }]]),
  ...
  await autoSymbolManager.maybeSwitchOnDistance({
    direction: 'LONG',
    monitorPrice: 20000,
    quotesMap: new Map([['NEW', { price: 0.1, lotSize: 100, name: 'NEW', symbol: 'NEW' }]]),
    positions: [],
    pendingOrders: [],
  });
...
```typescript
import { getTradingMinutesSinceOpen, getHKDateKey } from '../../utils/helpers/tradingTime.js';
...
async function maybeSwitchOnDistance({
  ...
  if (pendingSwitch) {
    ...
    if (!currentSeatState.symbol || currentSeatState.status === 'EMPTY') {
      switchStates.delete(direction);
      logger.warn(`[自动换标] 席位已清空，终止待处理换标: ${monitorSymbol} ${direction}`);
      return;
    }
