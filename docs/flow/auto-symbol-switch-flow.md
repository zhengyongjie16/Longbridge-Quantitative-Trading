...
- 覆盖自动寻标开启后的换标路径，包含同标的日内抑制；不包含启动时的初始寻标。
...
graph TD
  ...
  G --|是|--> G1{"当日抑制命中?"}
  G1 --|是|--> Z["停止换标（当日抑制）"]
  G1 --|否|--> H["预寻标候选<br/>findBestWarrant"]
  H --> H1{"候选 == 旧标的?"}
  H1 --|是|--> H2["标记当日抑制<br/>停止换标"]
  H1 --|否|--> I["clearSeat: 状态=SWITCHING<br/>seatVersion++"]
  I --> I1["清理方向队列/延迟验证<br/>清空牛熊证缓存"]
  I1 --> J0["创建 SwitchState<br/>缓存候选"]
  J0 --> J["processSwitchState"]
  J --> K{"是否存在旧标的买入挂单?"}
  K --|是|--> K1["批量撤销买入挂单"]
  K1 --> K2{"撤单是否全部成功?"}
  ...
  K2 --|是|--> L
  K --|否|--> L
  L{"可用持仓数量是否大于 0?"} --|是|--> L1["提交移仓卖出(ELO)"]
  L1 --> L2["等待成交<br/>下一轮继续处理"]
  L --|否|--> M{"总持仓大于 0 且可用为 0?"}
  M --|是|--> L2
  M --|否|--> N["读取最新卖出记录<br/>计算卖出资金"]
  N --> O["读取缓存候选"]
  O --> P{"候选标的是否为空?"}
  P --|是|--> X1
  P --|否|--> Q["写入候选标的<br/>席位保持 SWITCHING"]
  Q --> R{"是否需要移仓回补?"}
  R --|否|--> S["换标完成"]
  R --|是|--> R1{"行情与手数是否就绪?"}
  ...
  R4 --> S
  S --> S1["席位置 READY"]
  S1 --> T["refreshSeatAfterSwitch<br/>刷新缓存"]
  T --> U["清理方向牛熊证缓存"]
  U --> V["全量订单刷新订单记录"]
...
- 距回收价百分比越界（包含等于边界）后，先检查当日抑制；命中则直接返回，不预寻标、不清席位。
- 未命中抑制才预寻标候选（`findBestWarrant`）；候选与旧标的一致则记录当日抑制并停止换标。
- 候选为空或不同才执行 `clearSeat`：席位标记为 `SWITCHING`，并递增 `seatVersion`，用于阻断旧信号，同时将候选缓存到 `SwitchState`。
...
- **候选读取**：使用缓存候选，若为空则置席位 `EMPTY`。
- **占位**：候选存在时写入新标的，但席位保持 `SWITCHING`。
- **移仓买入**（可选）：
  - 若旧标的有持仓则 `shouldRebuy=true`，等待新标的行情就绪后按卖出资金（或目标金额）计算买入量。
  - 买入量有效则提交移仓买入（ELO），完成后将席位置为 `READY` 并结束换标状态机。
...
### 4. 日内抑制与跨日清理
- 抑制记录按“方向 + 标的 + 港股日期键”存放，避免同标的当日重复触发换标。
- 主循环跨日后刷新交易日信息（strict 模式）并清理抑制记录，保证次日可重新寻标。
