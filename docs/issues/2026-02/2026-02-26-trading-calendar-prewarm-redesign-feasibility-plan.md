...
1. **起点语义错配**：交易日历预热应服务于“当前仍持仓订单”的交易时长计算，但现状使用“历史全量最早订单”，把大量已平仓订单误纳入需求窗口。

2. **时序错配**：在 `loadTradingDayRuntimeSnapshot` 阶段尚未完成按席位/方向的订单重建，无法直接得到“当前仍持仓订单集合”，却提前决定预热窗口。

3. **接口约束错配**：分块大小 `180` 天与交易日接口单次区间限制不匹配，导致服务端拒绝。
