...
/**
 * 将原始K线数据转换为平均K线（Heikin Ashi）
 * @param {number[]} opens 开盘价数组
 * @param {number[]} highs 最高价数组
 * @param {number[]} lows 最低价数组
 * @param {number[]} closes 收盘价数组
 * @param {number[]} volumes 成交量数组
 * @param {number[]} timestamps 时间戳数组（可选）
 * @returns {{opens: number[], highs: number[], lows: number[], closes: number[], volumes: number[]}} 转换后的平均K线数据
 */
function convertToHeikinAshi(
  ...
  try {
    ...
    if (
      ...
      throw new Error("K线数据数组长度不一致");
    }

    const heikinAshiResult = HeikinAshi.calculate({
      open: opens,
      high: highs,
      low: lows,
      close: closes,
    ...
    // 验证转换结果
    if (
      !heikinAshiResult?.open ||
      !heikinAshiResult?.high ||
      !heikinAshiResult?.low ||
      !heikinAshiResult?.close ||
      heikinAshiResult.open.length !== length
    ) {
      throw new Error("平均K线转换结果无效");
    }

    return {
      opens: heikinAshiResult.open,
      highs: heikinAshiResult.high,
      lows: heikinAshiResult.low,
      closes: heikinAshiResult.close,
      volumes: heikinAshiResult.volume || volumes,
    };
  } catch (err) {
    // 如果转换失败，返回原始数据
    console.warn(`平均K线转换失败，使用原始K线数据：`, err?.message ?? err);
    return {
      opens,
      highs,
      lows,
 ...
 * 使用平均K线数据进行计算
 * @param {number[]} opens 开盘价数组（原始K线）
 * @param {number[]} highs 最高价数组（原始K线）
 * @param {number[]} lows 最低价数组（原始K线）
 * @param {number[]} closes 收盘价数组（原始K线）
 * @param {number[]} volumes 成交量数组
 * @param {number} candlesLength K线数量
...
function calculateAllIndicators(
  ...
  // 先计算平均K线
  const heikinAshi = convertToHeikinAshi(opens, highs, lows, closes, volumes);

  // 使用平均K线数据计算指标
  return {
    rsi6: calculateRSI(heikinAshi.closes, 6),
    rsi12: calculateRSI(heikinAshi.closes, 12),
    kdj: calculateKDJIndicator(
      heikinAshi.closes,
      heikinAshi.highs,
      heikinAshi.lows,
      candlesLength
    ),
    macd: calculateMACDIndicator(heikinAshi.closes),
    mfi: calculateMFIIndicator(
      heikinAshi.highs,
      heikinAshi.lows,
      heikinAshi.closes,
      heikinAshi.volumes
...
async function getIntradayCandlesticks(symbol, dateRangeStr) {
  try {
    ...
    const allOpens = candlesticks.map((c) => decimalToNumber(c.open));
    ...
    for (let i = rangeStartIndex; i < candlesticks.length; i++) {
      ...
      const opens = allOpens.slice(0, i + 1);
